...
- [`on__`](../../reference/jsx-attributes/on-star.md): adds an event listener to the `document` and dispatches it to the `element`. This can be referred to as a *delegated event*.
...
Instead of attaching event listeners to every individual element, Solid uses *synthetic event delegation*, through the [`on__`](../../reference/jsx-attributes/on-star.md) form . In this method, event listeners are attached to the `document` element and dispatch events to the relevant elements as they bubble up.
