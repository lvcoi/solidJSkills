...
- **Automatic data revalidation:** After an action successfully completes, Solid Router revalidates relevant [`queries`](../data-fetching/queries.md), ensuring the UI reflects the latest data.
- **Progressive enhancement:** When used with HTML forms, actions enable functionality even if JavaScript is not yet loaded.

* * *

## [Defining actions](actions.md#defining-actions)

Actions are defined by wrapping the data-mutation logic with the [`action` function](../reference/data-apis/action.md).
...
## [Using actions](actions.md#using-actions)

Actions can be triggered in two ways: using a HTML [`<form>` element](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/form) or programmatically using the [`useAction` primitive](../reference/data-apis/use-action.md).
...
For cases where a form is not suitable, the [`useAction` primitive](../reference/data-apis/use-action.md) can be used to trigger the action programmatically.

### [With the `<form>` element](actions.md#with-the-form-element)
...
#### [Passing additional arguments](actions.md#passing-additional-arguments)
...
### [With the `useAction` primitive](actions.md#with-the-useaction-primitive)
...
## [Tracking submission state](actions.md#tracking-submission-state)

When an action is triggered, it creates a **submission** object. This object is a snapshot of the action's execution, containing its input, current status (pending or complete), and its final result or error. To access this state, Solid Router provides the [`useSubmission`](../reference/data-apis/use-submission.md) and [`useSubmissions`](../reference/data-apis/use-submissions.md) primitives.
...
To track multiple submissions for a single action, such as in a multi-file uploader interface, the [`useSubmissions` primitive](../reference/data-apis/use-submissions.md) can be used.

* * *

## [Handling errors](actions.md#handling-errors)
...
## [Automatic data revalidation](actions.md#automatic-data-revalidation)

After server data changes, the application's can become stale. To solve this, Solid Router automatically revalidates all [queries](../data-fetching/queries.md) used in the same page after a successful action. This ensures any component using that data is automatically updated with the freshest information.
...
## [Managing navigation and revalidation](actions.md#managing-navigation-and-revalidation)
...
### [Redirecting](actions.md#redirecting)

To navigate the user to a new page after an action completes, the [`redirect` helper](../reference/response-helpers/redirect.md) can be used. It can also be used to revalidate specific queries upon redirection, which is useful for updating data that is displayed on the new page.
...
### [Customizing revalidation](actions.md#customizing-revalidation)

To override the default revalidation behavior, the [`reload`](../reference/response-helpers/reload.md) and [`json`](../reference/response-helpers/json.md) helpers can be used.
...
## [Optimistic UI](actions.md#optimistic-ui)
...
[Report an issue with this page](https://github.com/solidjs/solid-docs-next/issues/new?assignees=ladybluenotes&labels=improve%20documentation%2Cpending%20review&projects=&template=CONTENT.yml&title=%5BContent%5D%3A&subject=%2Fsolid-router%2Fconcepts%2Factions.mdx&page=https%3A%2F%2Fdocs.solidjs.com%2Fsolid-router%2Fconcepts%2Factions)

Previous[← Alternative routers](alternative-routers.md)

Next[Single page applications →](../rendering-modes/spa.md)
...
3. [Using actions](#using-actions)
   ...
   1. [With the &lt;form&gt; element](#with-the-form-element)
